        ...
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"Translate the following email into Dutch. Keep formatting intact if possible: {text}"}
                ],
                temperature=0.3
            )
            return response.choices[0].message.content
        except APIError as e:
            print(f"[Attempt {attempt}] OpenAI API error: {e}. Retrying in {delay} seconds...")
            time.sleep(delay)
